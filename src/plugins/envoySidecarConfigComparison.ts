...
export function compareEnvoyConfigs(onStreamOutput, configs1: any[], configs2: any[], type: string, 
                                  transform: boolean, itemKey: string, sidecarItemKey: string = itemKey, ignoreValues?: string[]) {
    ...
    if(key.includes("dynamic")) {
      envoyConfig2[key].forEach(c2 => {
        ...
        if(items[name]) {
          ...
          const matches = JsonUtil.compareObjects(config1Item, config2Item, itemDiffs, ignoreValues)
          if(matches) {
            matchingRecords.push(name)
            delete items[name]
          } else {
  ...
  if(Object.keys(items).length > 0) {
    ...
    Object.keys(items).forEach(item => {
      ...
      output.push(["<<","Differing Keys:",""], ["<<", diffs[name]])
    })
  } else {
...
const plugin : ActionGroupSpec = {
  ...
  actions: [
    {
      name: "Envoy Sidecar Config Comparison",
      order: 30,
      ...
      async act(actionContext) {
        ...
        this.onOutput && this.onOutput([["Sidecar Config Comparison", ""]], ActionOutputStyle.Log)
        ...
        const ipPrefixesToIgnore: string[] = []
        pod1IP && ipPrefixesToIgnore.push(pod1IP)
        pod2IP && ipPrefixesToIgnore.push(pod2IP)
        compareEnvoyConfigs(this.onStreamOutput, sidecar1Configs, sidecar2Configs, EnvoyConfigType.Clusters, false, "cluster")

        compareEnvoyConfigs(this.onStreamOutput, sidecar1Configs, sidecar2Configs, EnvoyConfigType.Listeners, false, "listener", "listener", ipPrefixesToIgnore)

        compareEnvoyConfigs(this.onStreamOutput, sidecar1Configs, sidecar2Configs, EnvoyConfigType.Routes, false, "route_config")
